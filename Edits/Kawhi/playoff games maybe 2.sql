...
replace(replace(description, ' ', '%20'), 'S.%20', ''), '", "', description, '")') Play,
...
from playByPlay pbp left join
        player p on pbp.player_idAST = p.player_id and p.season_id = 2021 left join
        game g on pbp.game_id = g.game_id
where p.name = 'Kawhi Leonard' or (description like '%(Leonard%' and description like '%AST)%')

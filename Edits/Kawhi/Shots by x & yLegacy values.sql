...
SELECT pbp.[game_id]
      ...
      ,[teamTricode] Tri,
concat('https://www.nba.com/stats/events?CFID=&CFPARAMS=&GameEventID=', actionNumber, '&GameID=00', pbp.game_id, '&Season=', pbp.season_id, '-', pbp.season_id - 2000 + 1, '&flag=1', '&title=', 
replace(REPLACE(replace(description, concat(Left(p.name, 1), '. '), ''), ' ', '%20'), 'S.%20', '')) Play,
concat('https://www.nba.com/game/', '...-vs-...', '-00', pbp.game_id, '/play-by-play?watchFullGame=true') Link
      ,shotDistance
      ...
      ,description
      ,cast(x as decimal(18,2)) trueX
      ,cast(y as decimal(18,2)) trueY
...
SELECT pbp.[game_id]
      ...
      ,[teamTricode] Tri,
concat('https://www.nba.com/stats/events?CFID=&CFPARAMS=&GameEventID=', actionNumber, '&GameID=00', pbp.game_id, '&Season=', pbp.season_id, '-', pbp.season_id - 2000 + 1, '&flag=1', '&title=', 
replace(REPLACE(replace(description, concat(Left(p.name, 1), '. '), ''), ' ', '%20'), 'S.%20', '')),
concat('https://www.nba.com/game/', '...-vs-...', '-00', pbp.game_id, '/play-by-play?watchFullGame=true') Link
      ,shotDistance
      ,case when x is null then xLegacy else cast(x as decimal(18,2)) end x
      ,case when y is null then yLegacy else cast(y as decimal(18,2)) end y

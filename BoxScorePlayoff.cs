...
using static NBAdb.Playoffs;

namespace NBAdb
{

    public class BoxScorePlayoff
    {
        public static BusDriver busDriver = new BusDriver();
        Playoffs playoffs = new Playoffs();
        public void Init(string seriesID, string firstGame, string lastGame, int season, int games)
        {
            var client = new WebClient { Encoding = System.Text.Encoding.UTF8 };
            int game = 1;
            for (int i = Int32.Parse(firstGame); i <= Int32.Parse(lastGame); i++)
            {
                string boxLink = "https://cdn.nba.com/static/json/liveData/boxscore/boxscore_00" + i.ToString() + ".json";
                try
                {
        ...
        public static void BoxPost(Root JSON, int season, string seriesID, int game)
        {
            using (SqlCommand InsertData = new SqlCommand("teamBoxPlayoffsInsert"))
            {
                InsertData.Connection = busDriver.SQLdb;
                InsertData.CommandType = CommandType.StoredProcedure;
                ...
                InsertData.Parameters.AddWithValue("@game", game);
                ...
                InsertData.ExecuteScalar();
                busDriver.SQLdb.Close();
                PlayerBoxInsert(JSON, 0, season, seriesID, game);
            }
            using (SqlCommand InsertDataAway = new SqlCommand("teamBoxPlayoffsInsert"))
            {
                InsertDataAway.Connection = busDriver.SQLdb;
                InsertDataAway.CommandType = CommandType.StoredProcedure;
                InsertDataAway.Parameters.AddWithValue("@season_id", season);
                InsertDataAway.Parameters.AddWithValue("@series_id", seriesID);
                InsertDataAway.Parameters.AddWithValue("@game_id", JSON.game.gameId);
                InsertDataAway.Parameters.AddWithValue("@game", game);
                ...
                busDriver.SQLdb.Close();
                PlayerBoxInsertAway(JSON, 0, season, seriesID, game);
            }
        }
        public static void PlayerBoxInsertAway(Root JSON, int updateCheck, int season, string seriesID, int game)
        {
            int players = JSON.game.awayTeam.players.Count;
            for (int i = 0; i < players; i++)
            {
                using (SqlCommand InsertDataAway = new SqlCommand("playerBoxPlayoffsInsert"))
                {
                    InsertDataAway.Connection = busDriver.SQLdb;
                    InsertDataAway.CommandType = CommandType.StoredProcedure;
                    InsertDataAway.Parameters.AddWithValue("@season_id", season);
        ...
        public static void PlayerBoxInsert(Root JSON, int updateCheck, int season, string seriesID, int game)
        {
            int players = JSON.game.homeTeam.players.Count;
            for (int i = 0; i < players; i++)
            {
                using (SqlCommand InsertData = new SqlCommand("playerBoxPlayoffsInsert"))
                {
                    InsertData.Connection = busDriver.SQLdb;
                    InsertData.CommandType = CommandType.StoredProcedure;
                    InsertData.Parameters.AddWithValue("@season_id", season);
